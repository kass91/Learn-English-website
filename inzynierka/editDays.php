...
if(isset($_POST['delete'])){
			$sqlCommand="DELETE FROM `days` WHERE IDDays='".mysqli_real_escape_string($myConnection, $_POST['IDDays'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
if(isset($_POST['edycja'])){
			$sqlCommand="UPDATE `days` SET `EN`='".$_POST['weren']."', `PL`='".$_POST['werpl']."' WHERE `IDDays`='".mysqli_real_escape_string($myConnection, $_POST['IDDays'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);	
			echo $sqlCommand;		           
	};
if(isset($_POST['dodaj'])){
			$sqlCommand="INSERT INTO `days` (`EN`,`PL`) values('".$_POST['english']."', '".$_POST['polish']."')"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
?>
        ...
        <div class="post">
            ...
            <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12"><?php
					...
					$sqlCommand="SELECT * FROM `days` WHERE IDDays='".mysqli_real_escape_string($myConnection, $_GET['id'])."'";			
                       ...
                       <br /><div class="col-xs-12"><form method="post" action="editDays.php"><input type="text" name="weren" value="<?php echo "$row[1]";?>" /> - <input type="text" name="werpl" value="<?php echo "$row[2]";?>" /> <input type="submit" name="edycja" value="zapisz"/> <input type="submit" name="delete" value="usuń"/><input type="hidden" value="<?php echo "$row[0]";?>" name="IDDays" /></form></div>
				   <?php } ?>
                   </div>
                   <div class="col-lg-5">
                   <form method="post" action="editDays.php">
