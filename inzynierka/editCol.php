...
if(isset($_POST['delete'])){
			$sqlCommand="DELETE FROM `colours` WHERE IDCol='".mysqli_real_escape_string($myConnection, $_POST['IDCol'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
if(isset($_POST['edycja'])){
			$sqlCommand="UPDATE `colours` SET `EN`='".$_POST['weren']."', `PL`='".$_POST['werpl']."' WHERE `IDCol`='".mysqli_real_escape_string($myConnection, $_POST['IDCol'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);	
			echo $sqlCommand;		           
	};
if(isset($_POST['dodaj'])){
			$sqlCommand="INSERT INTO `colours` (`EN`,`PL`) values('".$_POST['english']."', '".$_POST['polish']."')"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
?>
...
<div class="container">
	<div id="blog" class="col-lg-12">
        <div class="post">
       		<div class='post-tit'>Kolory</div>
            <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12"><?php
   					$myConnection= mysqli_connect("localhost","root","", "learnenglish") or die ("could not connect to mysql"); 
					if (isset($_GET['id']))
					$sqlCommand="SELECT * FROM `colours` WHERE IDCol='".mysqli_real_escape_string($myConnection, $_GET['id'])."'";			
					else
                        $sqlCommand="SELECT * FROM `colours`";
                        $guery=mysqli_query($myConnection, "set names 'utf8'");
                        $query=mysqli_query($myConnection, $sqlCommand);            
                        while($row = mysqli_fetch_array($query)){ ?>
                       <br /><div class="col-xs-12"><form method="post" action="editCol.php"><input type="text" name="weren" value="<?php echo "$row[1]";?>" /> - <input type="text" name="werpl" value="<?php echo "$row[2]";?>" /> <input type="submit" name="edycja" value="zapisz"/> <input type="submit" name="delete" value="usuń"/><input type="hidden" value="<?php echo "$row[0]";?>" name="IDCol" /></form></div>
				   <?php } ?>
                   </div>
                   <div class="col-lg-4">
                   <form method="post" action="editCol.php">
