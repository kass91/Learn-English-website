...
if(isset($_POST['delete'])){
			$sqlCommand="DELETE FROM `verbs` WHERE `IDVerb`='".mysqli_real_escape_string($myConnection, $_POST['id'])."'"; 
...
if(isset($_POST['zapisz'])){
			...
			$sqlCommand="UPDATE `verbs` SET `tresc`='".$tresc."', `przyklady`='".$przyklady."' WHERE `IDVerb`='".mysqli_real_escape_string($myConnection, $_POST['id'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);	         
	};
if(isset($_POST['dodaj'])){
			...
			$sqlCommand="INSERT INTO `verbs` (`nazwa`,`tresc`,`przyklady`) VALUES ('".$nazwa."', '".$tresc."', '".$przyklady."')"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
?>
...
<div class="container">
	<div id="blog" class="col-lg-12">
        <div class="post">
       		<div class='post-tit'>Czasowniki - edycja</div>
			...
			<?php
                    ...
                    $sqlCommand="SELECT * FROM `verbs`";
					$guery=mysqli_query($myConnection, "set names 'utf8'");
					$query=mysqli_query($myConnection, $sqlCommand);     
					     
					$ver = '';
					while($row = mysqli_fetch_array($query)){ ?>
                    	<div id="ver<?php echo $row['IDVerb']; ?>" class="ver_details" style="display:none;">
                           <form method="post" action="editVerb.php"><div class="col-xs-12">
                           <?php $ver .= '<option value="'.$row['IDVerb'].'">'.$row['nazwa'].'</option>'; ?>
                           ...
                           <input type="hidden" value="<?php echo $row['IDVerb'];?>" name="id" />
                           <div class="col-xs-12">
				   ...
				   <?php } ?>
                       <div class="col-xs-12" id="divSelectVer">
                           <select style="width:100%" id="ver">
                           <?php echo $ver; ?>
                           </select>
                       </div>
                   </div>
                   <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                   <form method="post" action="editVerb.php">
                  	 <div class="col-xs-12">
 ...
 <script type="text/javascript">
 	$('#divSelectVer').detach().prependTo($('.post > .col-lg-7'));
