...
if(isset($_POST['delete'])){
			$sqlCommand="DELETE FROM `czasy` WHERE id='".mysqli_real_escape_string($myConnection, $_POST['id'])."'"; 
...
if(isset($_POST['zapisz'])){
			...
			$sqlCommand="UPDATE `czasy` SET `tresc`='{$tresc}', `przyklady`='{$przyklady}' WHERE `id`='".mysqli_real_escape_string($myConnection, $_POST['id'])."'"; 
			$query=mysqli_query($myConnection, $sqlCommand);	         
	};
if(isset($_POST['dodaj'])){
			...
			$sqlCommand="INSERT INTO `czasy` (`nazwa`,`tresc`,`przyklady`) VALUES ('".$nazwa."', '".$tresc."', '".$przyklady."')"; 
			$query=mysqli_query($myConnection, $sqlCommand);			           
	};
?>
...
<div class="container">
	<div id="blog" class="col-lg-12 pull-right">
        <div class="post">
       		<div class='post-tit'>Czasy - edycja</div>
            <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12 czasy-edycja">
			<?php
   					$myConnection= mysqli_connect("localhost","root","", "learnenglish") or die ("could not connect to mysql"); 
					if (isset($_GET['id']))
					$sqlCommand="SELECT * FROM `czasy` WHERE id='".mysqli_real_escape_string($myConnection, $_GET['id'])."'";	
					else
                    $sqlCommand="SELECT * FROM `czasy`";
					$guery=mysqli_query($myConnection, "set names 'utf8'");
					$query=mysqli_query($myConnection, $sqlCommand);     
					     
					$tenses = '';
					while($row = mysqli_fetch_array($query)){ ?>
                    	<div id="tenses<?php echo $row['id']; ?>" class="tenses_details" style="display:none;">
                           <form method="post" action="editCzasy.php"><div class="col-xs-12">
                           <?php $tenses .= '<option value="'.$row['id'].'">'.$row['nazwa'].'</option>'; ?>
				   ...
				   <?php } ?>
                       <div class="col-xs-12" id="divSelectTenses">
                           <select style="width:100%" id="tenses">
                           <?php echo $tenses; ?>
                           </select>
                       </div>
                   </div>
                   <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                   <form method="post" action="editCzasy.php">
                  	 <div class="col-xs-12">
 ...
 <script type="text/javascript">
 	$('#divSelectTenses').detach().prependTo($('.post > .col-lg-7'));
	$('select#tenses').change(function() {
		var el = $('#tenses'+$(this).val());
		if (el.length > 0) {
			$('.tenses_details').hide();
			el.show();		
		}
