...
<div class="container">
	<div id="blog" class="col-lg-12">
        <div class="post3">
            ...
            <div class="post-tit3">Czas - Time</div>
            <div class="post-tre3">
            	<blockquote>Słownictwo związane z nazwami dni, miesięcy oraz słownictwem opisującym przedział czasowy.</blockquote><br /><br />
                 <div class="panel-group">
                <div class="panel panel-default">
                  <div class="panel-heading">
                    <h4 class="panel-title"> <a data-toggle="collapse" href="#collapse1">Dni</a></h4>
                  </div>
                  <div id="collapse1" class="panel-collapse collapse">
					...
					if (isset($_GET['id']))
					$sqlCommand="SELECT * FROM days WHERE IDDays='".mysqli_real_escape_string($myConnection, $_GET['id'])."'";			
					else
                        $sqlCommand="SELECT * FROM days";
                        $guery=mysqli_query($myConnection, "set names 'utf8'");
                        $query=mysqli_query($myConnection, $sqlCommand);            
                        while($row = mysqli_fetch_array($query)){ ?>
                       <br /><i><?php echo "$row[1]";?></i> - <?php echo "$row[2]";?>
				   <?php } ?>
                  </div>
                </div>
                </div>
                <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title"> <a data-toggle="collapse" href="#collapse2">Miesiące</a></h4>
                  </div>
                  <div id="collapse2" class="panel-collapse collapse">
                    <div class="panel-body">
					<?php
    $myConnection= mysqli_connect("localhost","root","", "learnenglish") or die ("could not connect to mysql"); 
                                $sqlCommand="SELECT * FROM months";
                                $guery=mysqli_query($myConnection, "set names 'utf8'");
                                $query=mysqli_query($myConnection, $sqlCommand);            
                                  while($row = mysqli_fetch_array($query)){ ?>

                    <br /><i><?php echo "$row[1]";?></i> - <?php echo "$row[2]";?> <?php } ?>
                  </div>
                </div>
                </div>
                <div class="panel panel-default">
               <div class="panel-heading">
                    <h4 class="panel-title"> <a data-toggle="collapse" href="#collapse3">Czas</a></h4>
                  </div>
    ...
    $myConnection= mysqli_connect("localhost","root","", "learnenglish") or die ("could not connect to mysql"); 
            ...
            <div class="panel-group">
                <div class="panel panel-default">
                  <div class="panel-heading">
                    <h4 class="panel-title"> <a data-toggle="collapse" href="#collapse4">Ćwiczenia</a></h4>
                  </div>
                  <div id="collapse4" class="panel-collapse collapse">
                    <div class="panel-body">
                    <?php
					$myConnection= mysqli_connect("localhost","root","", "learnenglish") or die ("could not connect to mysql"); 
					$sqlCommand="SELECT * FROM wyniki WHERE `typ`='4' AND `IDUser`='{$_SESSION['id']}' ORDER BY `data` DESC LIMIT 5";
					$guery=mysqli_query($myConnection, "set names 'utf8'");
					$query=mysqli_query($myConnection, $sqlCommand);            
					$set = false;
					$wyniki = '';
                    ...
                    <form method="post" action="checkExercise.php?type=4">
					<?php
					$sqlCommand="SELECT * FROM exe WHERE `typ`='4'";
					$guery=mysqli_query($myConnection, "set names 'utf8'");
					$query=mysqli_query($myConnection, $sqlCommand);            
					$i = 1;
					while($row = mysqli_fetch_array($query)){ 
